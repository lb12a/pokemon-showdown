...
* **Move and ability icons** are still placeholders. Pokemon and item art is
  real (see 9g); a signature move or an ability has no picture of its own yet,
  which only shows in the dex. See `assets/README.md`.
...
## 9f. The battle you actually watch

Until now a battle was a list of sentences appearing all at once next to two
small cards. It is now paced and animated, and the client is split in three:

| file | what it is |
| --- | --- |
| `server/static/fakemon.js` | the shell: connection, teambuilder, dex |
| `server/static/battle-fx.js` | every effect, drawn on one canvas - no image files |
| `server/static/battle.js` | the battle room: the queue, the protocol, the hover cards |

**Nothing is applied on arrival.** The server sends a whole turn at once, so
each protocol line is turned into a *step* and queued; a player drains the queue
at a readable pace, running each step's animation and writing its sentence. Three
rules follow from that and hold throughout `battle.js`: a handler may only
queue, every line gets a sentence, and the controls appear only once the queue is
empty - so you never choose a move while the last turn is still playing. A speed
picker (slow / normal / fast / instant) scales every beat.

**54 attack animations** come out of two tables rather than 54 hand-written
ones: `TYPES` says what a type looks like (its colours, the glyph its particles
are drawn as, how they move), and `CATEGORIES` says how Physical, Special and
Status are staged - a lunge and an impact, a projectile or beam that crosses the
gap, an aura that rises. One entry from each combines, so Fire always reads as
Fire and a Special always reads as a Special, and adding a type or restaging a
category is a few lines. Send-outs, faints, Mega Evolution, stat changes, status,
healing, items, abilities, weather, terrain, rooms and hazards each have their
own staging on top.

**Nothing happens unexplained.** The client has a branch for 85 protocol
commands, and a test plays real battles, collects every command they produced,
and fails if any of them would have gone unnarrated.

**Hover cards** show your own Pokémon and moves in full. An opponent shows only
what you could work out yourself: name, types, level, HP percent, the moves it
has actually used, the abilities it *could* have (and the exact one once it has
triggered), the item once it has shown itself - and stat *ranges*, from an
uninvested hindering spread up to a fully invested boosted one, never the exact
numbers.

## 9g. Artwork

Both sets of images are generated by scripts in `tools/fakemon/art/`, so neither
needs hand work when the dex changes.

* **189 Pokémon.** `install-pokemon-art.py` takes a folder of PNGs named after
  the species, trims each one's transparent border so they all fill their box
  the same way, and writes a 320px battle sprite and a 64px teambuilder icon
  under the project's ID naming rule.
* **240 items.** `make_items.py` draws each icon from the item's own name and
  effect: a word in the name picks one of ~59 shapes in `shapes.py`, and a theme
  word ("volcanic", "glacial", "static", "haunted", …) picks the palette;
  anything unthemed gets a hue of its own so no two items in a bag look alike.
  `pixel.py` is the little pixel canvas they draw on - grid, outline pass,
  nearest-neighbour upscale.

`node tools/fakemon/check.js` reports 0 warnings now: every Pokémon and every
item has real art.

## 9h. Handing a bot a team sheet

A team carries a `Bot Order` setting - `free` (the normal AI) or `fixed`. Fixed
means a bot given that team leads with the top Pokémon, sends the next one down
whenever something faints, and never switches by choice. It travels with the
team: the export text carries a `Bot Order: Fixed` line before the first
Pokémon, the importer reads it wherever it sits, and the client hands it to the
server as `order=fixed` alongside the packed team.
...
npx mocha                        # the suite (runs everything: 2465 tests)
